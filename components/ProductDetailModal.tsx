...
const ProductDetailModal: React.FC<Props> = ({ product, isOpen, onClose, onAddToCart, onBuyNow }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-0 overflow-hidden">
      ...
      <div 
        className="absolute inset-0 bg-brand-secondary/90 backdrop-blur-2xl transition-opacity animate-in fade-in duration-500" 
        onClick={onClose} 
      />
      
      <article className="relative w-full max-w-7xl bg-white rounded-t-[2.5rem] lg:rounded-[3rem] shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col lg:flex-row h-[100dvh] lg:h-[90vh] animate-in slide-in-from-bottom-full duration-500 ease-out">
        ...
        <div className="absolute top-0 left-0 right-0 z-50 flex justify-between items-center p-4 lg:p-8 pointer-events-none">
          ...
          <div className="flex gap-2 pointer-events-auto">
            <button className="p-3.5 bg-white/95 backdrop-blur-md rounded-2xl text-brand-secondary shadow-xl active:scale-90 transition-all border border-stone-100 hover:text-brand-accent">
        ...
        {/* 1. Image Section - Dynamic height on mobile */}
        <div className="w-full lg:w-[48%] h-[42vh] lg:h-auto relative bg-brand-light flex-shrink-0 group overflow-hidden">
          <img 
            src={product.image} 
            alt={product.name} 
          ...
          <div className="absolute bottom-6 left-6 flex flex-wrap gap-2 lg:top-12 lg:left-12 lg:bottom-auto">
            <div className="bg-brand-accent px-4 py-1.5 rounded-full shadow-lg border border-white/20 flex items-center gap-2">
              <Zap className="w-3 h-3 text-brand-secondary fill-current" />
              <span className="text-[10px] font-black uppercase tracking-widest text-brand-secondary">Tuyệt phẩm</span>
            </div>
            <div className="bg-white/95 backdrop-blur-md px-4 py-1.5 rounded-full shadow-lg border border-stone-100 flex items-center gap-2">
              <ShieldCheck className="w-3 h-3 text-green-600" />
              <span className="text-[10px] font-black uppercase tracking-widest text-brand-secondary">Đã khử độc</span>
            </div>
          </div>
        </div>

        {/* 2. Content Section - Scrollable with safe area for bottom bar */}
        <div className="w-full lg:w-[52%] flex flex-col h-full bg-white overflow-y-auto no-scrollbar relative">
          <div className="px-6 py-8 lg:p-16 pb-48"> {/* pb-48 ensures content isn't hidden by action bar */}
            ...
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] font-black text-stone-300 uppercase tracking-[0.2em]">5.0 (240+ lượt mua)</span>
              </div>
            ...
            {/* Quick Benefits - Two columns */}
            <div className="grid grid-cols-2 gap-2 mb-8">
               {product.benefits.map((benefit, i) => (
                 <div key={i} className="flex items-center gap-3 p-3.5 bg-stone-50/80 rounded-2xl border border-stone-100">
                    <div className="w-8 h-8 rounded-xl bg-green-500 text-white flex items-center justify-center flex-shrink-0 shadow-md shadow-green-100">
            ...
            <div className="min-h-[200px] animate-in fade-in duration-500">
              {activeTab === 'desc' && (
                <div className="space-y-6">
                  ...
                  <div className="p-5 bg-brand-accent/5 rounded-2xl border border-brand-accent/10 flex items-start gap-4">
                     ...
                     <p className="text-[11px] font-bold text-brand-secondary italic">"Thanh Hà cam kết 100% thảo dược tự nhiên, được sơ chế theo phương pháp bí truyền để giữ trọn dược tính."</p>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'usage' && (
                <div className="relative p-7 rounded-[2.5rem] border border-brand-accent/20 bg-gradient-to-br from-brand-light to-white overflow-hidden shadow-inner">
                  ...
                  <div className="relative z-10 space-y-8">
                    ...
                    <div className="space-y-6">
                      {[
                        { 
                          ...
                          desc: "Sử dụng từ 1-2 chén nhỏ (khoảng 20-30ml) trong mỗi bữa ăn chính để kích thích tiêu hóa.",
                          icon: <Beaker className="w-4 h-4" />
                        },
                        { 
                          num: "02", 
                          title: "Ướp lạnh hoàn hảo", 
                          desc: "Rượu sẽ ngon hơn khi được làm lạnh ở nhiệt độ 10-15°C trước khi thưởng thức.",
                          icon: <Wine className="w-4 h-4" />
                        },
                        { 
                          num: "03", 
                          title: "Bảo quản đúng cách", 
                          desc: "Đậy kín nắp sau khi dùng, để nơi khô ráo, tránh ánh nắng trực tiếp từ mặt trời.",
                    ...
                    <div className="p-4 bg-brand-secondary text-white rounded-2xl text-[10px] font-bold text-center italic shadow-2xl">
                      "Tinh hoa từ thiên nhiên - Thưởng thức có trách nhiệm."
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="mt-12 p-6 rounded-[2.5rem] bg-brand-light border border-brand-accent/10">
               <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-[10px] font-black text-brand-secondary uppercase tracking-widest">Ưu đãi Combo</h4>
                      <p className="text-[9px] text-stone-400 font-bold">Thường mua cùng sản phẩm này</p>
                    </div>
                  </div>
          ...
          {/* 3. FIXED ACTION BAR - Sticky at bottom with Blur */}
